  ...
  void handleJson(String json, int indentationSpace) {
    ...
    for (int i = 0; i < jsonData.entries.length; i++) {
      if (jsonData.entries.toList()[i].value is Map<String, dynamic>) {
        ...
        yamlText += Utils.addSpaces("label: ${Utils.handleNumericValueForYaml(Utils.camelCaseToEnglish(jsonData.entries.toList()[i].key))}", indentationSpace + 2) + "\n";
        yamlText += Utils.addSpaces("description: ${Utils.handleNumericValueForYaml(Utils.camelCaseToEnglish(jsonData.entries.toList()[i].key))}", indentationSpace + 2) + "\n";
        ...
        if (jsonData.entries.toList()[i].value[0] is Map<String, dynamic>) {
          ...
          yamlText += Utils.addSpaces("pattern: ${Utils.validationPattern(jsonData.entries.toList()[i].value[0].toString())}", indentationSpace + 4) + "\n";
          yamlText += Utils.addSpaces("message: ${Utils.validationMessage(jsonData.entries.toList()[i].value[0].toString())}", indentationSpace + 4) + "\n";
        ...
        if (jsonData.entries.toList()[i].value is bool) {
          ...
          yamlText += Utils.addSpaces("validations:", indentationSpace + 2) + "\n";
          yamlText += Utils.addSpaces("required: true", indentationSpace + 4) + "\n";
          yamlText += Utils.addSpaces("pattern: ${Utils.validationPattern(jsonData.entries.toList()[i].value.toString())}", indentationSpace + 4) + "\n";
          yamlText += Utils.addSpaces("message: ${Utils.validationMessage(jsonData.entries.toList()[i].value.toString())}", indentationSpace + 4) + "\n";
        }
    ...
    return Scaffold(
      body: Container(
        ...
        child: Column(
          children: [
            SizedBox(height: 10,),
            Text("JGC Yaml Creator", style: TextStyle(fontSize: 40, fontWeight: FontWeight.bold),),
            SizedBox(height: 10,),
            Row(
                      ...
                      child: Container(
                        ...
                        child: TextField(
                          ...
                          decoration: InputDecoration(
                            ...
                            hintText: "Paste your json text over here"
                          ),
                        )
                      ),
                    ),
                      ...
                      children: [
                        SizedBox(
                          ...
                          child: MaterialButton(
                            ...
                            color: Colors.red,
                            child: Text("Clear Yaml", style: TextStyle(color: Colors.white),),
                          ),
                        ),
                        SizedBox(width: 10,),
                        SizedBox(
                          ...
                          child: MaterialButton(
                            ...
                            color: Colors.black,
                            child: Text("Copy Yaml", style: TextStyle(color: Colors.white),),
                          ),
                        ),
                      ],
                    )
